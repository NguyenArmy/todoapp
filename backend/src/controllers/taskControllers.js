...
export const getAllTasks = async (req, res) => {
    ...
    switch (filter) {
        case "today":
            ...
            break;
        case "week": {
            const mondayDate = (now.getDay() - now.getDay() + 1) % 7 === 0 ? now.getDay() - 6 : now.getDate() - now.getDay() + 1;
    ...
    try {
        const result = await Task.aggregate([
...

            {
                ...
                $facet: {
                    ...
                    completeCount: [{ $match: { status: { $in: ["completed", "complete"] } } }, { $count: "count" }],
        ...
        console.error("loi khi goi  getAAllTasks", error);
